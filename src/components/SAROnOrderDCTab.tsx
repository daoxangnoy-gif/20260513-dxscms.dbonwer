...
export default function SAROnOrderDCTab() {
  const { toast } = useToast();
  const [rows, setRows] = useState<OnOrderDCRow[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const all: OnOrderDCRow[] = [];
      let offset = 0;
      const PAGE = 1000;
      while (true) {
        const { data, error } = await supabase
          .from("on_order_dc")
          .select("id,sku_code,qty,store_name,product_name_la,product_name_en,created_at")
          .order("created_at", { ascending: false })
          .range(offset, offset + PAGE - 1);
        if (error) throw error;
        const batch = (data || []) as OnOrderDCRow[];
        all.push(...batch);
  ...
  const downloadTemplate = () => {
    const ws = XLSX.utils.json_to_sheet([
      { "skucode/barcode": "0001234", qty: 100, store_name: "Jmart Sikai" },
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Template");
    XLSX.writeFile(wb, "OnOrderDC_Template.xlsx");
  };
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const k of Object.keys(sample)) {
        const n = norm(k);
        if (n === "skucode" || n === "sku" || n === "barcode" || n === "mainbarcode" || n === "skucodebarcode" || n === "barcodeskucode")
          keyMap[k] = "key";
        else if (n === "qty" || n === "quantity") keyMap[k] = "qty";
        else if (n === "storename" || n === "store") keyMap[k] = "store_name";
      }
      const kKey = Object.keys(keyMap).find(k => keyMap[k] === "key");
      const qKey = Object.keys(keyMap).find(k => keyMap[k] === "qty");
      const sKey = Object.keys(keyMap).find(k => keyMap[k] === "store_name");

      if (!kKey || !qKey || !sKey) {
        toast({ title: "Import Error", description: "ต้องมีคอลัมน์: skucode/barcode, qty, store_name", variant: "destructive" });
        return;
      }

      type Item = { key: string; qty: number; store: string };
      const items: Item[] = [];
      for (const r of json) {
        const key = String(r[kKey] ?? "").trim();
        const qty = Number(r[qKey]);
        const store = String(r[sKey] ?? "").trim();
        if (!key || !store || !Number.isFinite(qty)) continue;
        items.push({ key, qty, store });
      }
      if (items.length === 0) {
      ...
      const { error: insErr } = await supabase.from("on_order_dc").insert(inserts);
  ...
  const deleteRow = async (id: string) => {
    ...
    const { error } = await supabase.from("on_order_dc").delete().eq("id", id);
  ...
  const deleteAll = async () => {
    ...
    const { error } = await supabase.from("on_order_dc").delete().neq("id", "00000000-0000-0000-0000-000000000000");
    if (error) { toast({ title: "Delete Error", description: error.message, variant: "destructive" }); return; }
    setRows([]);
    setSelected(new Set());
  ...
  const exportRows = (list: OnOrderDCRow[], suffix: string) => {
    if (list.length === 0) {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "OnOrderDC");
    const ts = new Date();
    const stamp = `${ts.getFullYear()}${String(ts.getMonth()+1).padStart(2,"0")}${String(ts.getDate()).padStart(2,"0")}_${String(ts.getHours()).padStart(2,"0")}${String(ts.getMinutes()).padStart(2,"0")}`;
    XLSX.writeFile(wb, `OnOrderDC_${suffix}_${stamp}.xlsx`);
  };
