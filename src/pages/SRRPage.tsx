...
const SRR_RPC_CACHE = new Map<string, { ts: number; rows: any[] }>();
...
const SRR_COLUMNS: { key: keyof SRRRow | "pack_size"; label: string; group?: string }[] = [
...
function formatCellValue(val: any, key: string): string {
  ...
  if (typeof val === "number") {
    if (val === 0) return "";
    // PO Cost Unit: preserve full precision up to 10 decimals (no forced rounding)
...
function recalcRow(row: SRRRow): SRRRow {
  // Defensive numeric coercion — older saved snapshots may miss newer fields (e.g. *_kokkok_fc)
  // resulting in undefined → NaN propagation through min/max/stock/avg sums.
  ...
  // Gap Store: clamp negative store stock to 0; gap when clamped <= tt_min
  const clampedStockStore = ttStockStore <= 0 ? 0 : ttStockStore;
  const gapStore = clampedStockStore <= ttMin ? ttMin - clampedStockStore : 0;
  // Gap DC: clamp negative DC stock to 0; gap when clamped <= dc_min
  ...
  // DOH uses the rounded avg (2 decimals) to match Excel display calculations
...
// Apply NUMBER format with 10 decimals to numeric columns in a worksheet so
// Excel does not auto-round the displayed value (e.g. PO Cost Unit precision).
export function applyHighPrecisionFormat(ws: XLSX.WorkSheet, columnNames: string[] = ["Products to Purchase/Unit Price"]) {
  if (!ws || !ws["!ref"]) return;
  const range = XLSX.utils.decode_range(ws["!ref"]);
  // Read header row (row 0) and find target column indices
