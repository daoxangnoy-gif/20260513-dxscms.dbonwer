...
-- Fix get_srr_data: join po_cost by (item_id, vendor) instead of item_id only
CREATE OR REPLACE FUNCTION public.get_srr_data(p_spc_names text[] DEFAULT NULL::text[], p_order_days text[] DEFAULT NULL::text[], p_vendor_codes text[] DEFAULT NULL::text[], p_item_types text[] DEFAULT NULL::text[], p_division_groups text[] DEFAULT NULL::text[], p_divisions text[] DEFAULT NULL::text[], p_departments text[] DEFAULT NULL::text[], p_sub_departments text[] DEFAULT NULL::text[], p_classes text[] DEFAULT NULL::text[], p_sub_classes text[] DEFAULT NULL::text[], p_sku_codes text[] DEFAULT NULL::text[])
...
AS $function$
  WITH vm AS (
    ...
    WHERE vendor_code IS NOT NULL
      AND (vendor_origin ILIKE '%lao%' OR vendor_origin ILIKE '%thai%')
  ...
  -- FIX: include vendor and dedup by (item_id, vendor) so we can join with dm.vendor_code
  pc AS (
    SELECT DISTINCT ON (item_id, vendor) item_id, vendor, moq, po_cost, po_cost_unit
    FROM po_cost
    WHERE item_id IN (SELECT sku_code FROM sku_set)
    ORDER BY item_id, vendor, updated_at DESC
  ),
  latest_doc AS (
    SELECT data
  ...
  -- FIX: join by both SKU and vendor
...
-- Fix get_srr_d2s_data: same change
CREATE OR REPLACE FUNCTION public.get_srr_d2s_data(p_spc_names text[] DEFAULT NULL::text[], p_order_days text[] DEFAULT NULL::text[], p_vendor_codes text[] DEFAULT NULL::text[], p_item_types text[] DEFAULT NULL::text[], p_division_groups text[] DEFAULT NULL::text[], p_divisions text[] DEFAULT NULL::text[], p_departments text[] DEFAULT NULL::text[], p_sub_departments text[] DEFAULT NULL::text[], p_classes text[] DEFAULT NULL::text[], p_sub_classes text[] DEFAULT NULL::text[], p_sku_codes text[] DEFAULT NULL::text[])
...
AS $function$
  ...
  -- FIX: include vendor and dedup by (item_id, vendor)
  pc AS (
    SELECT DISTINCT ON (item_id, vendor) item_id, vendor, moq, po_cost, po_cost_unit
    FROM po_cost
    WHERE item_id IN (SELECT sku_code FROM sku_set)
