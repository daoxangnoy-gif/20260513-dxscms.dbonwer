-- get_sar_data_full — optimize: ดัน filter เข้า CTE
-- เดิม dm/price/rk/sales_agg/rsv scan ทั้งตาราง master ทุก call (ช้า ~10s)
-- ใหม่: ทำ mm0 (minmax filtered) ก่อน แล้ว CTE อื่นประมวลเฉพาะ sku/store ใน mm0
-- signature เดิม (12 args) → CREATE OR REPLACE ทับได้เลย
...
AS $function$
  WITH mm0 AS (
    -- filter ที่อยู่บน column ของ minmax โดยตรง → ตัดขนาดก่อน join master
    SELECT m.item_id, m.store_name, COALESCE(m.type_store,'') AS type_store,
           m.unit_pick, m.min_val, m.max_val
  ...
  sku_keys AS (SELECT DISTINCT item_id AS sku_code FROM mm0),
  store_keys AS (SELECT DISTINCT store_name FROM mm0),
  dm AS (
    ...
    WHERE sku_code IN (SELECT sku_code FROM sku_keys)
    ORDER BY sku_code, updated_at DESC NULLS LAST
  ),
  price AS (
    SELECT DISTINCT ON (sku_code) sku_code, standard_price, list_price, jmart_price
    FROM public.data_master
    WHERE sku_code IN (SELECT sku_code FROM sku_keys) AND packing_size_qty = 1
    ORDER BY sku_code, updated_at DESC NULLS LAST
  ),
  rk AS (
    SELECT DISTINCT ON (item_id) item_id, final_rank
    FROM public.rank_sales
    WHERE item_id IN (SELECT sku_code FROM sku_keys)
  ...
  rsv_per_store AS (
    ...
    WHERE rsv.sku_code IN (SELECT sku_code FROM sku_keys)
  ...
  sales_agg AS (
    ...
    WHERE sw.id18 IN (SELECT sku_code FROM sku_keys)
      AND sw.store_name IN (SELECT store_name FROM store_keys)
  ...
  base AS (
    ...
    WHERE (p_item_types      IS NULL OR cardinality(p_item_types)=0      OR COALESCE(d.item_type,'') = ANY(p_item_types))
...
-- Index ช่วยเร่ง join (idempotent — ถ้ามีแล้วข้าม)
CREATE INDEX IF NOT EXISTS idx_minmax_item_store ON public.minmax (item_id, store_name);
CREATE INDEX IF NOT EXISTS idx_sales_by_week_id18_store ON public.sales_by_week (id18, store_name);
