-- 1) Switch get_minmax_calc_all (5-arg) to read from range_store_view instead of range_store_snapshots
...
-- 2) Drop legacy 1-arg overload (no longer used)
DROP FUNCTION IF EXISTS public.get_minmax_calc_all(numeric);

-- 3) Drop the snapshots table entirely (Save Document feature removed)
DROP TABLE IF EXISTS public.range_store_snapshots CASCADE;