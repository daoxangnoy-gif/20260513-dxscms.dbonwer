...
AS $function$
  ...
  mm_agg AS (
    SELECT sku,
      MAX(min_val) FILTER (WHERE store_name ILIKE 'Jmart%' OR type_store = 'Jmart') AS min_jmart,
      MAX(max_val) FILTER (WHERE store_name ILIKE 'Jmart%' OR type_store = 'Jmart') AS max_jmart,
      MAX(min_val) FILTER (WHERE (store_name ILIKE 'Kokkok%' AND store_name NOT ILIKE '%Fc%' AND store_name NOT ILIKE '%FC%') OR type_store = 'Kokkok') AS min_kokkok,
      MAX(max_val) FILTER (WHERE (store_name ILIKE 'Kokkok%' AND store_name NOT ILIKE '%Fc%' AND store_name NOT ILIKE '%FC%') OR type_store = 'Kokkok') AS max_kokkok,
      MAX(min_val) FILTER (WHERE store_name ILIKE '%Kokkok%Fc%' OR store_name ILIKE '%Kokkok-FC%' OR type_store = 'Kokkok-Fc') AS min_kokkok_fc,
      MAX(max_val) FILTER (WHERE store_name ILIKE '%Kokkok%Fc%' OR store_name ILIKE '%Kokkok-FC%' OR type_store = 'Kokkok-Fc') AS max_kokkok_fc,
      MAX(min_val) FILTER (WHERE store_name ILIKE 'U-dee%' OR type_store = 'U-dee') AS min_udee,
      MAX(max_val) FILTER (WHERE store_name ILIKE 'U-dee%' OR type_store = 'U-dee') AS max_udee
