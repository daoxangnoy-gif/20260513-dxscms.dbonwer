-- Modify get_minmax_calc_all to use unit_pick_override as primary source for unit_pick
-- Falls back to minmax.unit_pick if no override exists, then defaults to 1
...
BEGIN
  ...
  v_sql := format($SQL$
    ...
    -- unit_pick_override: overrides unit_pick per SKU × store (managed in Unit Pick tab)
    up_over AS (
    ...
    SELECT
      ...
      -- unit_pick: override first, fallback to minmax saved value, then 1
      ...
      (CASE WHEN COALESCE(sps.avg_sale,0)=0 THEN 3
       ...
       END)::numeric,
      -- max_cal: CEIL((min_cal + avg * N) / unit_pick) * unit_pick
