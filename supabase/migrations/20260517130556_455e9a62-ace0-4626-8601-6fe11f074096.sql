...
AS $function$
  WITH
  -- Step 1: filter stores FIRST (only 59 rows total)
  store_meta AS (
  ...
  -- Step 2: filter SKUs FIRST from data_master (push all SKU-level filters down)
  master AS (
    ...
    FROM data_master dm
    WHERE dm.product_owner = 'Lanexang Green Property Sole Co.,Ltd'
  ...
  -- Step 3: expand jsonb ONLY for filtered SKUs and filter store_name early
  range_y AS (
    SELECT
      rsv.sku_code,
...
-- Index to support range_store_view ↔ master JOIN
CREATE INDEX IF NOT EXISTS idx_rsv_sku ON public.range_store_view(sku_code);
