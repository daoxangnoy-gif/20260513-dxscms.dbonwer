-- 1) SRR DC: AVG → SUM for avg_sales per type_store
...
-- 2) Min/Max View RPC: read from latest Doc instead of live minmax table
CREATE OR REPLACE FUNCTION public.get_minmax_view_by_stores(
  ...
  p_barcodes text[] DEFAULT NULL::text[]
)
RETURNS TABLE(sku_code text, product_name_la text, product_name_en text, main_barcode text, unit_of_measure text, store_name text, type_store text, unit_pick numeric, min_val numeric, max_val numeric, item_type text, buying_status text, division text, department text, sub_department text, class text, pack_qty numeric, box_qty numeric)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
AS $function$
  WITH latest_doc AS (
    SELECT data FROM public.minmax_cal_documents ORDER BY created_at DESC LIMIT 1
  ),
  doc_rows AS (
    SELECT
      ...
      COALESCE(r->>'type_store','')::text AS type_store,
      NULLIF(r->>'unit_pick','')::numeric AS unit_pick,
  ...
  SELECT
    ...
    dr.store_name, dr.type_store,
    dr.unit_pick,
    dr.min_val, dr.max_val,
  ...
  FROM doc_rows dr
  LEFT JOIN dm d ON d.sku_code = dr.sku_code
  LEFT JOIN rsv_per_store ry ON ry.sku_code = dr.sku_code AND ry.store_name = dr.store_name
  WHERE (p_stores         IS NULL OR cardinality(p_stores)=0         OR dr.store_name = ANY(p_stores))
    AND (p_type_stores    IS NULL OR cardinality(p_type_stores)=0    OR dr.type_store = ANY(p_type_stores))
    ...
    AND (p_skus           IS NULL OR cardinality(p_skus)=0           OR dr.sku_code = ANY(p_skus))
