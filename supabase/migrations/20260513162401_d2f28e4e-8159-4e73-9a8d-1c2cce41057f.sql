...
AS $function$
  WITH master AS (
    ...
    WHERE dm.stock_unit_flag = 'Y'
      ...
      AND dm.product_owner = 'Lanexang'
      AND COALESCE(dm.buying_status, '') <> 'Inactive'
...
-- Also patch sync_range_store_view_skus to apply same filters
...
-- Re-sync now to apply filters
TRUNCATE public.range_store_view;
INSERT INTO public.range_store_view
SELECT d.*, now() FROM public.get_range_store_data() d;